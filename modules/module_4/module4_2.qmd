...
Let's start by watching [a video](https://www.youtube.com/watch?v=MDniRwXizWo&ab_channel=AugmentedStartups) that does a nice job explaining the basics of KNN.
...
Okay, we have determined that our best k value is 5. Now we can use our training model on the testing data to see if it makes any mistakes. We use the `predict` function from `caret` to do this.
...
Typically, you would want to replace the columns in the entire dataframe, but we will end here for today :)

Our goal of labeling our unlabeled collars is complete! We will deploy any that are now labeled as Collarium Inc.