...
a.  a data frame with the average growth of fish per diet treatment (`perc_soy_protein`); growth is being measured by the change in weight of each fish after 30 days on the diet (`day_30_weight`).
...
**NOTE! We need to run the line of code below. This line of code tells R to treat the soy protein column as a category rather than a number, and to display the levels in order from lowest to highest.**

```{r}
growth <- growth %>% 
  mutate(perc_soy_protein = factor(perc_soy_protein, levels = c(0.2, 0.4, 0.6, 0.8)))
```
...
9.  Make a multiple scatter plot with the average water temperature in the tank on the x-axis (the actual number, not whether the tank is "cold" or "warm") and the amount of growth on the y-axis (vertical). Change the color of each point so that they represent the amount of soy protein in the diet. Change the axis labels to be more easily understood and add a theme. (2 point)
...
10. Imagine we had decided to treat `perc_soy_protein` as a continuous variable. What type of plot would we have used for task (b)? Would we have been able to complete task (d)? Why or why not? (2 points)
